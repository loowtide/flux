...
Tokenizer::Tokenizer() { loadStopWords(); }
...
void Tokenizer::makeToken(std::vector<Token> &tokens, std::string &word,
                          uint32_t &currentPos, const uint32_t &pageNo) {
  if (word.empty())
    return;
  Token token;
  token.text = word;
...
void Tokenizer::tokenizeLine(fz_stext_line *line, std::vector<Token> &tokens,
                             uint32_t &currentPos, const uint32_t &pageNo) {
  std::string currWord;
...
void Tokenizer::tokenizeStextPage(fz_stext_page *stext_page,
                                  std::vector<Token> &tokens,
                                  uint32_t &currentPos,
                                  const uint32_t &pageNo) {
...
void Tokenizer::tokenizePage(fz_context *ctx, fz_document *doc, int pageIndex,
                             std::vector<Token> &tokens, uint32_t &currentPos) {
...
std::vector<Token> Tokenizer::tokenize(const std::string &file_path) {
  ...
  fz_catch(ctx) { std::cerr << "PDF extraction failed" << std::endl; }
  fz_drop_context(ctx);
  return tokens;
}
